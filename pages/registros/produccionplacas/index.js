...
const ProduccionPlacas = () => {
    ...
    useEffect(() => {
        ...
        if (isLoadingMoreRef.current) {
            ...
            setRegistros(prev => [...prev, ...data.obtenerRegistrosPP]);
        } else {
            setRegistros(data.obtenerRegistrosPP);
        }
    }, [data]);
            ...
            <div className="flex justify-between">
                ...
                <div>
                    <button onClick={() => handleOpenClose(setFiltros, filtros)}>
                        <a className="bg-blue-800 py-2 px-5 mt-1 ml-1 inline-block text-white rounded text-sm hover:bg-gray-800 mb-3 uppercase font-bold w-full lg:w-auto text-center">Buscar</a>
                    </button>
                    {rol === "Admin" ? 
                        <button onClick={() => handleOpenClose(setPdfOpen, pdfOpen)}>
                            <a className="bg-blue-800 py-2 px-5 mt-1 ml-1  inline-block text-white rounded text-sm hover:bg-gray-800 mb-3 uppercase font-bold w-full lg:w-auto text-center">Exportar en pdf</a>
                        </button>
                    : null}
                </div>
            </div>

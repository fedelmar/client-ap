...
import {
  OBTENER_REGISTRO,
  ACTUALIZAR_REGISTRO,
} from "../../../../servicios/preparacionDeGel";
...
const EditarRegistro = () => {
  ...
  const { pid: id } = query;
  const [actualizarRegistroPG] = useMutation(ACTUALIZAR_REGISTRO, {
    refetchQueries: ['obtenerRegistrosPG'],
  ...
  useEffect(() => {
    if (data) {
      const { obtenerRegistroPG } = data;
      setRegistro(obtenerRegistroPG);
    }
  }, [data]);
  ...
  const finalizar = async (valores) => {
    const { cantidad, lote } = valores;
    Swal.fire({
      title: "Verifique los datos antes de confirmar",
      html: "Lote: " + lote + "</br>" + "Cantidad: " + cantidad + "</br>",
      icon: "warning",
      ...
      if (result.value) {
        try {
          const { data } = await actualizarRegistroPG({
            variables: {
              id: id,
              input: {
                lote,
      ...
      <div className="flex justify-center mt-5">
        <div className="w-full bg-white shadow-md px-8 pt-6 pb-8 mb-4 max-w-lg">
          <div className="mb-2 border-b-2 border-gray-600">
            ...
            <div className="flex">
              <p className="text-gray-700 text-mm font-bold mr-1">Llenado: </p>
              <p>{registro.llenado ? "✔" : "✘"}</p>
            </div>
            <div className="flex pb-2">
              <p className="text-gray-700 text-mm font-bold mr-1">
                Lote de Insumo:
              </p>
              <p className="text-gray-700 font-light ">{registro.loteInsumo}</p>
            </div>
          </div>

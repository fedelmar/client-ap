...
import { LISTA_REGISTROS_POR_FECHA } from '../../../servicios/guardadoDePlacas';
...
const RegistrosPorFecha = ({start, end, setRegs}) => {
    ...
    useEffect(() => {
      if (data) setRegs([...data.getRegsByDateGP]);
    },[data]);
    ...
    return (
      <>
        {data.getRegsByDateGP.length > 0 ? 
          <p className="text-gray-800 font-light ml-2 mr-2 mt-1" >Se obtuvieron {data.getRegsByDateGP.length} registros</p>
        :

...
import { LISTA_REGISTROS_POR_FECHA } from '../../../servicios/ingresos';
...
const RegistrosPorFecha = ({start, end, setRegs}) => {
    ...
    useEffect(() => {
      if (data) setRegs([...data.getRegsByDateIngreso]);
    },[data]);
    ...
    return (
      <>
        {data.getRegsByDateIngreso.length > 0 ? 
          <p className="text-gray-800 font-light ml-2 mr-2 mt-1" >Se obtuvieron {data.getRegsByDateIngreso.length} registros</p>
        :
